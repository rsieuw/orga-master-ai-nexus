...
Deno.serve(async (req: Request) => {
  ...
  const languagePreference = 'nl'; // Declare with default here
  console.log("[deep-research] Function invoked, processing request");

  try {
    // Store the raw request body for debugging
    const rawBody = await req.text();
    console.log("[deep-research] Raw request body:", rawBody);
    
    // Parse JSON manually after consuming the request body
    let requestData;
    try {
      requestData = JSON.parse(rawBody);
      console.log("[deep-research] Parsed request data:", JSON.stringify(requestData));
    ...
    if (requestData.testParam) {
      console.log("[deep-research] Test payload detected:", requestData.testParam);
      return new Response(
        JSON.stringify({ 
          id: "test-response-id",
          status: "success", 
    ...
    // Log API key status voor debugging (zonder de echte keys te tonen)
    console.log("[deep-research] API key status - Perplexity:", perplexityApiKey ? "aanwezig" : "ontbreekt");
    console.log("[deep-research] API key status - OpenAI:", openAIApiKey ? "aanwezig" : "ontbreekt");
...
Respond in ${reqLanguagePreference === 'nl' ? 'Dutch' : 'English'}.`;
    ...
    console.log(`[deep-research] Making AI request to ${apiEndpoint} with model ${apiModel}`);
    ...
    if (!apiResponse.ok) {
      ...
      if (apiResponse.status === 401 || apiResponse.status === 403) {
        console.log("[deep-research] Authentication error, falling back to mock response for testing");
    ...
    console.log("[deep-research] AI response received", JSON.stringify(aiResult).substring(0, 200) + "...");
    ...
    console.log("[deep-research] Returning successful response");
...
function generateMockResponse(query: string, mode: string) {
  console.log("[deep-research] Generating mock response as fallback");
